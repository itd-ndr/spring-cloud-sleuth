...
import static io.micrometer.tracing.instrument.reactor.ReactorSleuth.scopePassingSpanOperator;
...
public abstract class ScopePassingSpanSubscriberTests {
	...
	void should_set_empty_context_when_exception_occurs_while_trying_to_retrieve_the_context() {
		ScopePassingSpanSubscriber<?> subscriber = new ScopePassingSpanSubscriber<>(null, Context.empty(),
				currentTraceContext(), null);
