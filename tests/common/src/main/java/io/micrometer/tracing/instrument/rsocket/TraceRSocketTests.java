...
package io.micrometer.tracing.instrument.rsocket;
...
public abstract class TraceRSocketTests {
	...
	public void should_instrument_requester_and_responder() throws Exception {
		...
		final io.micrometer.tracing.Tracer tracer = context
				.getBean(io.micrometer.tracing.Tracer.class);
		final TestSpanHandler spans = context.getBean(TestSpanHandler.class);
		final int port = context.getBean(Environment.class).getProperty("local.server.port", Integer.class);
		final TestController controller2 = context.getBean(TestController.class);
		...
		// REQUEST FNF
		final io.micrometer.tracing.Span nextSpanFnf = tracer.nextSpan().start();
		...
		// REQUEST RESPONSE
		final io.micrometer.tracing.Span nextSpanRR = tracer.nextSpan().start();
		...
		// REQUEST STREAM
		final io.micrometer.tracing.Span nextSpanRS = tracer.nextSpan().start();
		...
		// REQUEST CHANNEL
		final io.micrometer.tracing.Span nextSpanRC = tracer.nextSpan().start();
