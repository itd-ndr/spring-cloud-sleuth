...
package io.micrometer.tracing.brave.instrument.async.issues.issue410;
...
import io.micrometer.tracing.instrument.async.LazyTraceExecutor;
...
class Issue410Tests {
	 ...
	 */
	@Test
	void should_pass_tracing_info_for_submitted_tasks_with_threadPoolTaskScheduler() {
		Span span = this.tracer.nextSpan().name("foo");
		try (Tracer.SpanInScope ws = this.tracer.withSpanInScope(span)) {
			log.info("Starting test");
	...
	@Test
	void should_pass_tracing_info_for_scheduled_tasks_with_threadPoolTaskScheduler() {
		Span span = this.tracer.nextSpan().name("foo");
		try (Tracer.SpanInScope ws = this.tracer.withSpanInScope(span)) {
			log.info("Starting test");
			String response = this.restTemplate
