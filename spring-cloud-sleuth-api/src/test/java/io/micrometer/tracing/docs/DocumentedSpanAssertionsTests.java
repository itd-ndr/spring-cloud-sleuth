...
class DocumentedSpanAssertionsTests {
	...
	void should_not_fail_when_keys_and_values_are_properly_prefixed() {
		...
		assertThatKeyIsValid(MyDynamicTags.A_DYNAMIC_TAG.getKey().formatted("some"), SPAN_WITH_DYNAMIC_ENTRIES);
		assertThatEventIsValid("some.value", SPAN_WITH_DYNAMIC_ENTRIES);
		assertThatEventIsValid(MyDynamicEvents.A_DYNAMIC_EVENT.getValue().formatted("some"),
