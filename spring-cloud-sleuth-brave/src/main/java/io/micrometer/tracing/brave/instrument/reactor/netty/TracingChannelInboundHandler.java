...
package io.micrometer.tracing.brave.instrument.reactor.netty;
...
public class TracingChannelInboundHandler extends ChannelInboundHandlerAdapter {
	...
	boolean instrumentOperation(ChannelHandlerContext ctx, Runnable operation) {
		...
		if (span != null) {
			...
			if (conn instanceof ConnectionObserver observer) {
				TraceContext parent = observer.currentContext().getOrDefault(TraceContext.class,
						null);
				if (parent != null) {
				...
				if (ops instanceof HttpClientResponse response) {
					TraceContext parent = TracingHandlerUtil
							.traceContext(response.currentContextView());
					if (parent != null) {
						try (CurrentTraceContext.Scope scope = currentTraceContext.maybeScope(parent)) {
							operation.run();
						}
