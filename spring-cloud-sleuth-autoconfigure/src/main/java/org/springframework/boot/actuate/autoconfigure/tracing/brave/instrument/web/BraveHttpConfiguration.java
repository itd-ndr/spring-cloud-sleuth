...
import org.springframework.boot.actuate.autoconfigure.tracing.instrument.web.ConditionalOnSleuthHttp;
import org.springframework.boot.actuate.autoconfigure.tracing.instrument.web.SleuthHttpProperties;
import org.springframework.boot.actuate.autoconfigure.tracing.instrument.web.SleuthWebProperties;
import io.micrometer.tracing.brave.bridge.BraveHttpRequestParser;
import io.micrometer.tracing.brave.bridge.BraveHttpResponseParser;
import io.micrometer.tracing.brave.bridge.BraveSamplerFunction;
import io.micrometer.tracing.brave.instrument.web.BraveSpanFromContextRetriever;
import io.micrometer.tracing.brave.instrument.web.CompositeHttpSampler;
import io.micrometer.tracing.brave.instrument.web.SkipPatternHttpClientSampler;
import io.micrometer.tracing.brave.instrument.web.SkipPatternHttpServerSampler;
...
import io.micrometer.tracing.instrument.web.SpanFromContextRetriever;
...
public class BraveHttpConfiguration {
	...
	private brave.http.HttpRequestParser toBraveHttpRequestParser(BeanFactory beanFactory, String beanName) {
		...
		brave.http.HttpRequestParser parser = bean instanceof brave.http.HttpRequestParser hrp
				? hrp
				: bean instanceof HttpRequestParser hrp ? BraveHttpRequestParser.toBrave(hrp) : null;
	...
	private brave.http.HttpResponseParser toBraveHttpResponseParser(BeanFactory beanFactory, String beanName) {
		Object bean = beanFactory.getBean(beanName);
		brave.http.HttpResponseParser parser = bean instanceof brave.http.HttpResponseParser hrp
				? hrp : bean instanceof HttpResponseParser hrp
						? BraveHttpResponseParser.toBrave(hrp) : null;
	...
	private SamplerFunction<brave.http.HttpRequest> toBraveSampler(BeanFactory beanFactory, String beanName) {
		...
		SamplerFunction<brave.http.HttpRequest> braveSampler = bean instanceof SamplerFunction
				...
				: bean instanceof io.micrometer.tracing.SamplerFunction ? BraveSamplerFunction.toHttpBrave(
						(io.micrometer.tracing.SamplerFunction<io.micrometer.tracing.http.HttpRequest>) bean)
						: null;
		return returnOrThrow(bean, braveSampler, beanName, SamplerFunction.class,
				io.micrometer.tracing.SamplerFunction.class);
	}
