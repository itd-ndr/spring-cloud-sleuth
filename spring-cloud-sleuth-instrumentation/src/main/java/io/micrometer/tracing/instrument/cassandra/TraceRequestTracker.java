...
package io.micrometer.tracing.instrument.cassandra;
...
import static io.micrometer.tracing.instrument.cassandra.SleuthCassandraSpan.CASSANDRA_SPAN;
import static io.micrometer.tracing.instrument.cassandra.SleuthCassandraSpan.Events.NODE_ERROR;
import static io.micrometer.tracing.instrument.cassandra.SleuthCassandraSpan.Events.NODE_SUCCESS;
import static io.micrometer.tracing.instrument.cassandra.SleuthCassandraSpan.Tags.NODE_ERROR_TAG;
...
enum TraceRequestTracker implements RequestTracker {
	...
	public void onSuccess(@NonNull Request request, long latencyNanos, @NonNull DriverExecutionProfile executionProfile,
		...
		if (request instanceof CassandraSpanSupplier supplier) {
			Span span = supplier.getSpan();
	...
	public void onNodeError(@NonNull Request request, @NonNull Throwable error, long latencyNanos,
		...
		if (request instanceof CassandraSpanSupplier supplier) {
			AssertingSpan span = AssertingSpan.of(CASSANDRA_SPAN, supplier.getSpan());
			span.event(NODE_ERROR);
			span.tag(NODE_ERROR_TAG.getKey().formatted(node.getEndPoint()), error.toString());
