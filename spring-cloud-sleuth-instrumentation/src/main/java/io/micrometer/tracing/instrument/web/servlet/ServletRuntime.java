...
abstract class ServletRuntime {
	...
	/**
	 * public for
	 * {@link io.micrometer.tracing.instrument.web.servlet.HttpServletResponseWrapper}.
	 */
	...
	public static void propagateIfFatal(Throwable t) {
		...
		else if (t instanceof ThreadDeath death) {
			throw death;
		}
		else if (t instanceof LinkageError error) {
			throw error;
		}
	}
	...
	static final class Servlet3 extends ServletRuntime {
		...
		static final class TracingAsyncListener implements AsyncListener {
			...
			public void onComplete(AsyncEvent e) {
				...
				if (sendHandled instanceof AtomicBoolean boolean1 && boolean1.compareAndSet(false, true)) {
