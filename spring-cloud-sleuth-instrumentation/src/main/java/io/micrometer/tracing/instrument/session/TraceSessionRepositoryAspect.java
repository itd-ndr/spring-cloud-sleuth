...
package io.micrometer.tracing.instrument.session;
...
public class TraceSessionRepositoryAspect {
	...
	private SessionRepository wrapSessionRepository(SessionRepository target) {
		if (target instanceof FindByIndexNameSessionRepository repository) {
			return new TraceFindByIndexNameSessionRepository(this.tracer, repository);
		}
