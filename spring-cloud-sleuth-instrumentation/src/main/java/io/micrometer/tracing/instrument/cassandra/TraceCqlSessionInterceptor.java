...
import io.micrometer.tracing.docs.AssertingSpanBuilder;
import io.micrometer.tracing.internal.ContextUtil;
...
class TraceCqlSessionInterceptor implements MethodInterceptor {
	...
	private static Statement<?> createStatement(Object[] args) {
		if (args[0] instanceof Statement statement) {
			return statement;
		}
		else if (args[0] instanceof String string && args.length == 1) {
			return SimpleStatement.newInstance(string);
		}
		else if (args[0] instanceof String query && args.length == 2) {
			return args[1] instanceof Map m ? SimpleStatement.newInstance(query, m)
					: SimpleStatement.newInstance(query, (Object[]) args[1]);
		}
		throw new IllegalArgumentException("Unsupported arguments %s".formatted(Arrays.toString(args)));
	}
